...
-- Indexes are string input/output names, values are the corresponding KEY_* enums
for k, v in ipairs(keynames) do
    nametoenum[v] = keyenums[k]
end
...
function ENT:NumpadActivate(key)
    if (self.toggle) then return self:Switch(self.Buffer[key] == 0, key) end
