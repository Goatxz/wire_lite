...
-- arrow keys
for i = 17, 20 do
    unprintable_chars[i] = true
end

-- backspace, numpad, ctrl, alt, shift, break, F1-F12, scroll/num/caps lock, and more
for i = 127, 177 do
    unprintable_chars[i] = true
end
...
local function getPrintableChar(key)
    ...
    -- convert newline '13' into newlne '10' to make it work properly
    if key == 13 then
        key = 10
    end
...
function ENT:PlayerAttach(ply)
    ...
    -- If the player is already using a different keyboard
    if IsValid(ply.WireKeyboard) then
...
function ENT:GetRemappedKey(key_enum)
    ...
    if IsValid(self.ply) then
        layout = self.ply:GetInfo("wire_keyboard_layout", "American")
    end
    ...
    if isstring(ret) then
        ret = utf8.codepoint(ret)
    end

    if not self.EnterKeyAscii and ret == 10 then
        ret = 13
    end
...
function ENT:KeyPressed(key_enum)
    ...
    if not All_Enums[key] then
        All_Enums[key] = key_enum
    end
...
function ENT:PushBuffer(key, key_enum)
    ...
    if not self.BufferLookup[key_enum] then
        self.BufferLookup[key_enum] = {}
    end
...
function ENT:Think()
    ...
    for key_enum, _ in pairs(self.ply.keystate) do
        if key_enum == leavekey then
            -- if LCONTROL and LALT are being pressed, then the player is trying to use the "ALT GR" key which is available for some languages
            if leavekey ~= KEY_LALT or not self:IsPressedEnum(KEY_LCONTROL) then
