...
hook.Add("EntityTakeDamage", "CheckWireDamageDetectors", function(ent, dmginfo)
    ...
    if not r then
        print("Wire damage detector error: " .. e)
    end
end)
...
function ENT:ClearEntities()
    for i = 1, #self.linked_entities do
        -- generally, all entities should be kept valid automatically by the CallOnRemove functions, but CallOnRemove isn't called for players apparently
...
function ENT:TriggerInput(iname, value)
    if iname == "On" then
        self.on = value ~= 0
    elseif iname == "Entities" then
        -- Populate linked_entities from "Array"
        if value then
            self:ClearEntities()
...
-- Entity outputs won't trigger again until they change
...
-- Check to see if prop is registered by the detector
function ENT:UpdateLinkedEnts()
    if #self.linked_entities == 0 then return end
    self.key_ents = {}

    -- include linked_entities
    for i = 1, #self.linked_entities do
        local ent = self.linked_entities[i]

        if IsValid(ent) then
            -- Don't update constrained entities unless we have to
            if self.includeconstrained == 1 then
...
-- Finds all entities constrained to 'ent'
function ENT:UpdateConstrainedEnts(ent)
...
-- Update damage table
function ENT:UpdateDamage(dmginfo, ent)
    local damage = dmginfo:GetDamage()

    -- Only register the first target's damage info
    if not self.hit then
        self.firsthit_dmginfo = {dmginfo:GetAttacker(), ent, dmginfo:GetDamagePosition(), dmginfo:GetDamageForce()}
    ...
    -- Explosives will output the entity that receives the most damage
    if self.dmgtype == "Explosive" then
    ...
    -- This shouldn't ever happen... unless you're really REALLY bored
    if self.count == math.huge then
        self.count = 0
    end
end
...
function ENT:ApplyDupeInfo(ply, ent, info, GetEntByID)
    ...
    if info.linked_entities then
        -- old dupe compatibility
        if type(info.linked_entities) == "number" then
