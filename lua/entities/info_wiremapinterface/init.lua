...
function ENT:KeyValue(key, value)
    ...
    if ((key == "onwireentscreated") or (key == "onwireentsremoved") or (key == "onwireentsready") or (key == "onwireentsstartchanging")) then
        self:StoreOutput(key, value)
    end
...
local function SplitTable(tab, self)
    ...
    for i = 1, #tab do
        ...
        if (Port) then
            ...
            -- Do not add ports with no names
            if (name) then
                Index = Index + 1
