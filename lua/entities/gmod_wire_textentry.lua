...
if CLIENT then
    ...
    net.Receive("wire_textentry_show", function()
        ...
        panel = Derma_StringRequest("Wire Text Entry", "Enter text below", "", function(text)
...
function ENT:Prompt(ply)
    if ply then
        ...
        net.Send(ply)
        self:UpdateOverlay()
    elseif IsValid(self.Vehicle) and IsValid(self.Vehicle:GetDriver()) then
        -- linked
...
hook.Add("PlayerLeaveVehicle", "wire_textentry_leave_vehicle", function(ply, vehicle)
    if vehicle.WireTextEntry and IsValid(vehicle.WireTextEntry) and IsValid(vehicle.WireTextEntry.User) and vehicle.WireTextEntry.User == ply then
