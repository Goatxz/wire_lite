...
if CLIENT then
    ...
    function FLIR.start()
        ...
        for hookName, materials in pairs(materialOverrides) do
            hook.Add("Pre" .. hookName, "flir", function()
                render.MaterialOverride(materials[1])
            end)

            hook.Add("Post" .. hookName, "flir", function()
                render.MaterialOverride(materials[2])
            end)
        end
    ...
    function FLIR.start(player)
        FLIR.enable(player, true)
    end

    function FLIR.stop(player)
        FLIR.enable(player, false)
    end
