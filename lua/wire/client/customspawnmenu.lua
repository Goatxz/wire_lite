...
local function expandall(bool, nodes)
    for i = 1, #nodes do
        if nodes[i].m_bExpanded ~= bool then
            ...
            if nodes[i].WireCookieText then
                cookie.Set(nodes[i].WireCookieText, bool and 1 or 0)
            end
        end
...
function PANEL:Init()
    ...
    -- too big! you won't be able to see the rest of the spawn menu if it's this big, let's make it smaller
    ...
    elseif width < 390 then
        -- too small! you won't be able to see the tools, make it bigger
        width = 390
    end

    -- things changed, update convars
    if width ~= tab_width:GetInt() then
...
function PANEL:SetupSearchbox()
    ...
    function self.SearchBox:OnEnter(select_next)
        ...
        -- if we have no lines at all, do nothing
        if lines > 0 then
            local line = parent.SearchList:GetSelectedLine() or 0

            -- if tabbed, select next line
            if select_next then
                if lines > line then
            ...
            elseif line == 0 then
                -- if not tabbed, only select first line if no line is selected
                parent.SearchList:OnClickLine(parent.SearchList:GetLine(1))
            end
        end
    end
    ...
    function self.SearchBox:OnLoseFocus()
        -- regain focus if tabbed
        if self.Tabbed then
            self:RequestFocus()
            self.Tabbed = nil
        else
            -- if empty, reset "Search..." text
        ...
        if text ~= "" then
            ...
            for i = 1, #results do
                ...
                function line:OnRightClick()
                    ...
                    if b and b == 1 then
                        menu:AddOption("Remove from favourites", function()
                            cookie.Set(self.WireFavouritesCookieText, 0)
                            parent:ReloadEverything()
                        end)
                    else
                        menu:AddOption("Add to favourites", function()
                            cookie.Set(self.WireFavouritesCookieText, 1)
                            parent:ReloadEverything()
                        end)
                    end
...
Changes will take effect 3 seconds after you edit the value.]])
    ...
    function TabWidth:ValueChanged(value)
        ...
        timer.Create("wire_tab_width_changed", 3, 1, function()
            ...
            -- change the width of all registered tabs
            for i = 1, #all_tabs do
