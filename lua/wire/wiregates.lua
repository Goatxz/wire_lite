...
gamt = {
    ...
    __index = function(t, k) return rawget(gates, k) end,
    __call = function(t, s)
        gamt.curcat = s or "DEFAULT"
    end --call the table to set a default category
}
...
local function UpdateBannedGates(reload_spawnmenu)
    ...
    if CLIENT and reload_spawnmenu then
        RunConsoleCommand("spawnmenu_reload")
    end
end

cvars.AddChangeCallback("wire_banned_gate_categories", function()
    UpdateBannedGates(true)
end, "UpdateBannedGates")
