...
_deserialize = {
    ...
    ["number"] = function(s, i, len, unnecessaryEnd, jobstate)
        ...
        --	Locals, locals, locals, locals
        a = find(s, "[;:}~]", i)
        if a then return tonumber(sub(s, i, a - 1)) or error("vON: Number definition does not contain a valid number!"), a - 1 end
        --	Using %D breaks identification of negative numbers. :(
        error("vON: Number definition started... Found no end.")
    end,
...
_serialize = {
    ...
    ["table"] = function(data, mustInitiate, isNumeric, isKey, isLast, first, jobstate)
        ...
        for k, v in next, data do
            ...
            --	k % 1 == 0 is, as proven by personal benchmarks,
            if type(k) ~= "number" or k < 1 or k > len or (k % 1 ~= 0) then
                keyvals[#keyvals + 1] = k --	the quickest way to check if a number is an integer.
            end
            --	k % 1 ~= 0 is the fastest way to check if a number
        end

        --	is NOT an integer. > is proven slower.
        keyvalsLen = #keyvals
    ...
    ["string"] = function(data, mustInitiate, isNumeric, isKey, isLast, first, jobstate)
        if sub(data, #data, #data) == "\\" then return "\"" .. gsub(data, "\"", "\\\"") .. "v\"" end --	Hah, old strings fix this best.
    ...
    --	Fastest.
    ["nil"] = function(data, mustInitiate, isNumeric, isKey, isLast, first, jobstate) return "@" end
}

--[[    This section handles additions necessary for Garry's Mod.    ]]
--	Luckily, a specific table named after the game is present in Garry's Mod.
if gmod then
    local Entity = Entity
    ...
    local extra_deserialize = {
        ...
        ["Entity"] = function(s, i, len, unnecessaryEnd, jobstate)
            ...
            --	Locals, locals, locals, locals
            a = find(s, "[;:}~]", i)
            if a then return Entity(tonumber(sub(s, i, a - 1))), a - 1 end
...
_d_meta = {
    __call = function(self, str, allowIdRewriting)
        if type(str) == "string" then return _d_table(str, nil, #str, true, {{}, allowIdRewriting}) end
