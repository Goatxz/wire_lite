...
if CLIENT then
    ...
    TOOL.Information = {
        ...
        {
            name = "reload",
            text = "Increase angle offset by 45 degrees"
        },
        {
            name = "reload_shift",
            text = "Shift+Reload: Unparent gate (If parented)"
        }
    }
    ...
    function TOOL.BuildCPanel(panel)
        ...
        local function Search(text)
            ...
            for action, gate in pairs(GateActions) do
                ...
                -- If it has ANY match at all
                if string_find(lowname, text, 1, true) then
                    results[#results + 1] = {
                        name = gate.name,
                        group = gate.group,
                        action = action,
        ...
        for gatetype, gatefuncs in pairs(WireGatesSorted) do
            ...
            if any_allowed then
                CategoriesSorted[#CategoriesSorted + 1] = {
                    gatetype = gatetype,
                    gatefuncs = allowed_gates
                }
            end
        end
        ...
        -- when you check the parent box, check the nocollide box
        function parentbox.Button:DoClick()
            self:Toggle()
