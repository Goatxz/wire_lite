...
if SERVER then
    ...
    function TOOL:GetConVars()
        return self:GetClientNumber("a"), math.Clamp(self:GetClientNumber("ar"), 0, 255), math.Clamp(self:GetClientNumber("ag"), 0, 255), math.Clamp(self:GetClientNumber("ab"), 0, 255), math.Clamp(self:GetClientNumber("aa"), 0, 255), self:GetClientNumber("b"), math.Clamp(self:GetClientNumber("br"), 0, 255), math.Clamp(self:GetClientNumber("bg"), 0, 255), math.Clamp(self:GetClientNumber("bb"), 0, 255), math.Clamp(self:GetClientNumber("ba"), 0, 255)
    end

    function TOOL:PostMake(ent)
        duplicator.StoreEntityModifier(ent, "material", {
            MaterialOverride = self:GetClientInfo("material")
        })
    end
end
...
function TOOL:GetGhostMin(min)
    ...
    if Model == "models/props_trainstation/trainstation_clock001.mdl" or Model == "models/segment.mdl" or Model == "models/segment2.mdl" then return min.x end
...
function TOOL.BuildCPanel(panel)
    ...
    panel:AddControl("ComboBox", {
        ...
        Options = {
            ...
            ["Shiny"] = {
                wire_indicator_material = "models/shiny"
            },
            ["Metal"] = {
                wire_indicator_material = "models/props_c17/metalladder003"
            }
        }
    })
