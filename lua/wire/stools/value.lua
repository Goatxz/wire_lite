...
if CLIENT then
    ...
    local validityChecks = {
        ...
        ["2D Vector"] = function(val)
            local x, y = string.match(val, "^ *([^%s,]+) *, *([^%s,]+) *$")

            return tonumber(x) ~= nil and tonumber(y) ~= nil
        end,
        Vector = function(val)
        ...
        ["4D Vector"] = function(val)
            local x, y, z, w = string.match(val, "^ *([^%s,]+) *, *([^%s,]+) *, *([^%s,]+) *, *([%d.]+) *$")
    ...
    local typeGuessing = {{1, validityChecks.Number}, {5, validityChecks["2D Vector"]}, {4, validityChecks.Vector}, {6, validityChecks["4D Vector"]}, {2, validityChecks.String}} -- we're not checking angle because it's indistinguishable from vectors -- dropdown position, function
    ...
    net.Receive("wire_value_values", function(length)
        ...
        if not IsValid(slider) then return end -- They right clicked without opening the cpanel first, just save the values
        loadValues()
    end)
    ...
    function TOOL.BuildCPanel(panel)
        ...
        typeGuessCheckbox:SetToolTip([[When enabled, the type dropdown will automatically be updated as you type with
...
types fail.]])
        ...
        valueSlider.OnValueChanged = function(valueSlider, value)
            ...
            if value ~= LastValueAmount then
                if value > LastValueAmount then
                    for i = LastValueAmount + 1, value do
                        panels[i] = AddValue(itemPanel, i)
                    end
                elseif value < LastValueAmount then
                    for i = value + 1, LastValueAmount do
                        selectedValues[i] = nil

                        if IsValid(panels[i]) then
                            panels[i]:Remove()
                        end
