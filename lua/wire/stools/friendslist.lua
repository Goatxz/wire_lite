...
if SERVER then
    ...
    hook.Add("OnEntityCreated", "wire_friendslist_created", function(ent)
        if ent:GetClass() == "gmod_wire_friendslist" then
            -- wait for wire to set the GetPlayer variable
    ...
    net.Receive("wire_friendslist", function(length)
        ...
        saveFile(friends)
        if not IsValid(cpanel_list) then return end -- They right clicked without opening the cpanel first, just save the values
        cpanel_list:Clear()
