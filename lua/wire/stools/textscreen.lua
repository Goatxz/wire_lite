...
if SERVER then
    ...
    function TOOL:GetConVars()
        return self:GetClientInfo("text"), (16 - tonumber(self:GetClientInfo("tsize"))), self:GetClientNumber("tjust"), self:GetClientNumber("valign"), self:GetClientInfo("tfont"), Color(math.Clamp(self:GetClientNumber("tred"), 0, 255), math.Clamp(self:GetClientNumber("tgreen"), 0, 255), math.Clamp(self:GetClientNumber("tblue"), 0, 255)), Color(math.Clamp(self:GetClientNumber("tbgred"), 0, 255), math.Clamp(self:GetClientNumber("tbggreen"), 0, 255), math.Clamp(self:GetClientNumber("tbgblue"), 0, 255))
    end
...
function TOOL.BuildCPanel(panel)
    local Fonts = {"WireGPU_ConsoleFont", "Coolvetica", "Arial", "Lucida Console", "Trebuchet", "Courier New", "Times New Roman", "ChatFont", "Marlett", "Verdana", "Tahoma", "HalfLife2", "HL2cross", "Trebuchet MS", "HL2MP"}
    local Options = {}

    for k, v in ipairs(Fonts) do
        Options[v] = {
            wire_textscreen_tfont = v
        }
    end
