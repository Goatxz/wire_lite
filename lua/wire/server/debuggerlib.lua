...
function formatPort.RANGER(value)
    return "ranger"
end
...
function formatPort.ARRAY(value, OrientVertical)
    ...
    for Index, Element in ipairs(value) do
        ...
        if (ElementCount > 10) then break end
        ...
        elseif ((istable(Element) and #Element == 3) or isvector(Element)) then
            --vector
            RetText = RetText .. formatPort.VECTOR(Element)
        elseif (istable(Element) and #Element == 2) then
            --vector2
            RetText = RetText .. formatPort.VECTOR2(Element)
        elseif (istable(Element) and #Element == 4) then
            --vector4
            RetText = RetText .. formatPort.VECTOR4(Element)
        elseif ((istable(Element) and #Element == 3) or isangle(Element)) then
            --angle
...
function formatPort.TABLE(value, OrientVertical)
    ...
    for Index, Element in pairs(value) do
        ElementCount = ElementCount + 1
        if (ElementCount > 7) then break end
        ...
        elseif (istable(Element) and #Element == 2) then
            --vector2
            RetText = RetText .. formatPort.VECTOR2(Element)
        elseif (istable(Element) and #Element == 4 and typeid == "v4") then
            --vector4
            RetText = RetText .. formatPort.VECTOR4(Element)
        elseif (istable(Element) and #Element == 3) or isangle(Element) then
            --angle
        ...
        elseif (typeid == "s") then
            --string
            RetText = RetText .. formatPort.STRING(Element)
        elseif (isentity(Element) and typeid == "e") then
            --entity
            RetText = RetText .. formatPort.ENTITY(Element)
        elseif (type(Element) == "Player") then
            --player
