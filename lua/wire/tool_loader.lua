...
if SERVER then
    ...
    function WireToolObj:MakeEnt(ply, model, Ang, trace)
        local ent = WireLib.MakeWireEnt(ply, {
        ...
        if ent.RestoreNetworkVars then
            ent:RestoreNetworkVars(self:GetDataTables())
        end

        return ent
    end

    function WireToolObj:GetConVars()
        return
    end

    function WireToolObj:GetDataTables()
        return {}
    end
    ...
    function WireToolObj:LeftClick_Update(trace)
        ...
        if trace.Entity.RestoreNetworkVars then
            trace.Entity:RestoreNetworkVars(self:GetDataTables())
        end
    end
    ...
    function WireToolObj:LeftClick_PostMake(ent, ply, trace)
        ...
        if self.PostMake then
            self:PostMake(ent, ply, trace)
        end
...
function WireToolObj:Think()
    ...
    if not IsValid(self.GhostEntity) or self.GhostEntity:GetModel() ~= model then
        -- the tool as a function for getting the proper angle for the ghost
        ...
        if IsValid(self.GhostEntity) and CLIENT then
...
function WireToolObj:GetModel()
    ...
    if self:CheckValidModel(model_convar) then return model_convar end --use a valid model or the server crashes :<
...
function WireToolObj:GetAngle(trace)
    ...
    elseif self.GhostAngle then
        -- the tool gives a fixed angle to add
...
if SERVER then
    WireToolObj.PostMake_SetPos = WireToolObj.SetPos
end
...
if CLIENT then
    ...
    function WireToolObj:DrawToolScreen(width, height)
        ...
        draw.DrawText("World Weld:  " .. (on and "On" or "Off"), "GmodToolScreen20", 5, height - 38, Color(on and 150 or 255, on and 255 or 150, 150, 255))
        local on = self:GetOwner():GetInfo("wire_tool_nocollide") ~= "0" and not self:GetOwner():KeyDown(IN_WALK)
        draw.DrawText("Nocollide All: " .. (on and "On" or "Off"), "GmodToolScreen20", 5, height - 22, Color(on and 150 or 255, on and 255 or 150, 150, 255))
    end
...
if CLIENT then
    ...
    function WireToolHelpers.GetTOOL(mode)
        for _, wep in ipairs(LocalPlayer():GetWeapons()) do
            if wep:GetClass() == "gmod_tool" then return wep:GetToolObject(mode) end
        end
    end
...
-- empty stub outside of Singleplayer
...
function WireToolSetup.SetupLinking(SingleLink, linkedname)
    ...
    function TOOL:RightClick(trace)
        ...
        -- stage 0: right-clicking on our own class selects it
        ...
        elseif self:GetStage() == 1 then
            -- stage 1: right-clicking on something links it
            ...
            if success then
                if self.SingleLink or not ply:KeyDown(IN_SPEED) then
                    self:SetStage(0)
                end
    ...
    function TOOL:Reload(trace)
        ...
        if self:CheckHitOwnClass(trace) then
            ...
            return true
        elseif self:GetStage() == 1 then
            -- stage 1: reloading on something else unlinks it
            local ply = self:GetOwner()
