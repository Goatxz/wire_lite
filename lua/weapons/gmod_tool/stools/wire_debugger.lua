...
--try to find out if the entity is wire
...
if CLIENT then
    function TOOL:DrawHUD()
        ...
        if inputs and #inputs ~= 0 then
            ...
            draw.RoundedBox(8, boxx - 8, boxy - 8, boxw + 16, boxh + 16, Color(109, 146, 129, 192))
...
if (SERVER) then
    ...
    local function updateForPlayer(ply, cmps)
        -- if player has left, clear the hud
...
if (CLIENT) then
    ...
    local function DebuggerDrawHUD()
        ...
        for _, Line in pairs(dbg_lines) do
            ...
            --break it into multible lines for 1 entry
            for Index, ExplodeLine in ipairs(ExplodeLines) do
                if string.Trim(ExplodeLine) ~= "" then
                    local XPos = 0

                    if (Index > 1) then
                        --if the string is not the first and it is vertical, line it up acordingly
                        if dgb_orient_vert then
